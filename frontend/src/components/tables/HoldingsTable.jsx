 ...
 * @param {{ holdings: Array, totalPrincipal: number, totalProjected: number, avgRate: number, onRemove?: (ticker:string)=>void }} props
 */
export default function HoldingsTable({ holdings = [], totalPrincipal = 0, totalProjected = 0, avgRate = 0, onRemove }) {
  return (
    <div className="flex flex-col flex-1 rounded-xl bg-[var(--bg-card)] border border-[var(--border-subtle)] overflow-hidden">
      {/* Header bar */}
      <div className="flex items-center justify-between px-6 py-4">
        <span className="font-inter text-base font-semibold text-[var(--text-primary)]">Holdings</span>
        <button className="flex items-center gap-2 px-3 py-1.5 rounded-md border border-[var(--border-default)] text-[var(--text-muted)]">
      ...
      {/* Table head */}
      <div className="flex items-center px-6 py-3 bg-[var(--bg-elevated)]">
        {COLS.map((col, i) => (
      ...
      {/* Rows */}
      {holdings.map((h) => (
        <div key={h.ticker} className="flex items-center px-6 py-3.5 border-t border-[var(--border-subtle)]">
          <div className="flex flex-col gap-0.5 flex-1">
            <span className="font-inter text-[13px] font-semibold text-[var(--text-primary)]">{h.ticker}</span>
            <span className="font-inter text-[11px] text-[var(--text-muted)]">{h.name}</span>
          </div>
          ...
          <div className="w-20 flex justify-end">
            <button onClick={() => onRemove?.(h.ticker)} className="text-[var(--text-muted)] hover:text-[var(--error)]">
              <Trash2 size={16} />
            </button>
          </div>
        </div>
      ...
      {/* Footer totals */}
      <div className="flex items-center px-6 py-3.5 bg-[var(--bg-elevated)] border-t border-[var(--border-subtle)]">
